    ...
    *results,
    ...
    if stream is not None and isinstance(stream, DriftStream):
        if not prevent_plotting_drifts:
            ...
            # Add horizontal line for each concept
            # drift_index = results[0]["drift_detected_index"]
            # if drift_index:
            #     for idx in drift_index:
            #         print(f"Drift detected at position: {idx}")
            #         plt.axvline(idx, color="green", linestyle="--")
